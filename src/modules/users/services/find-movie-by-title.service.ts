...
import { MovieEntity, TMDBRepositoryInterface } from "../repositories/interfaces/tmdb-repository.interface";
  ...
  implements FindMoviesByTitleServiceInterface
{
  ...
  async execute(title: string): Promise<MovieEntity[]> {
    const movieFromDatabase = await this.movieRepository.findMoviesByTitle(title);
